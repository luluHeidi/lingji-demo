...
class ModuleLoader {
    ...
    bindMenuEvents() {
        ...
        // M13: Support level-1 menus with data-submenu-direct (no submenu, load module directly)
        const directMenus = document.querySelectorAll('.menu-item.level-1[data-submenu-direct]');
        directMenus.forEach(menu => {
            menu.addEventListener('click', (e) => {
                const submenuId = menu.dataset.submenuDirect;
                const moduleId = this.getModuleIdBySubmenu(submenuId);
                if (moduleId) {
                    document.querySelectorAll('.menu-item.level-2').forEach(m => m.classList.remove('active'));
                    document.querySelectorAll('.menu-item.level-1').forEach(m => m.classList.remove('active'));
    ...
    getModuleIdBySubmenu(submenuId) {
        ...
        const mapping = {
            ...
            // 二创权应用（M13菜单重构）
            'project-management': 'module-auth-management',
            'rights-review': 'module-auth-management',
            'auth-available-list': 'module-auth-available-list',
            'export-center': 'module-export-center',
    ...
    async loadModule(moduleId) {
        ...
        // 每次重新加载模块（不使用缓存，确保获取最新内容和重新初始化）
        if (this.modules.has(moduleId)) {
            this.modules.delete(moduleId);
        }
        // 移除旧的 JS script 标签，确保模块 init 函数被重新执行
        var oldScript = document.querySelector(`script[data-module="${moduleId}"]`);
    ...
    async loadModuleJS(moduleId, jsPath) {
        return new Promise((resolve, reject) => {
            // 如果已加载，移除旧 script 后重新加载（确保 init 被重新触发）
            var oldScript = document.querySelector(`script[data-module="${moduleId}"]`);
            if (oldScript) oldScript.remove();
