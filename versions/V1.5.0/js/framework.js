...
function initSidebarMenu() {
    ...
    menuGroups.forEach(group => {
        ...
        level1Menu.addEventListener('click', function(e) {
            ...
            // 如果没有子菜单且没有直接模块绑定，显示开发中提示
            if ((!submenu || submenu.children.length === 0) && !level1Menu.dataset.submenuDirect) {
        ...
        level2Menus.forEach(menu => {
            menu.addEventListener('click', function(e) {
                ...
                // 如果模块加载器尚未初始化完成，显示开发中提示作为回退
                // 模块加载器就绪后会自行处理内容渲染
                if (!window.moduleLoader || !window.moduleLoader.registry) {
                    showUnderDevelopment();
                }
            });
        });
