  ...
  container.innerHTML = `
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px 20px;margin-bottom:16px">
      <div style="display:flex;flex-wrap:wrap;gap:12px 16px">
        ...
        <div class="rr-filter-group-v2">
          <label>作品名称</label>
          <input type="text" class="rr-filter-sel" id="tabAFilterName" placeholder="多条用逗号分隔" value="${tabAFilters.play_name}" style="width:140px">
        </div>
        <div class="rr-filter-group-v2">
          <label>作品类型</label>
        ...
        <div class="rr-filter-group-v2">
          <label>授权起始</label>
          <input type="date" class="rr-filter-sel" id="tabAFilterStart" value="${tabAFilters.auth_start}" style="width:130px">
        </div>
        <div class="rr-filter-group-v2">
          <label>授权截止</label>
          <input type="date" class="rr-filter-sel" id="tabAFilterEnd" value="${tabAFilters.auth_end}" style="width:130px">
        </div>
        <div class="rr-filter-group-v2">
          <label>IP等级</label>
          ...
          ${pageData.map(item => {
            ...
            return `
              <tr>
                ...
                <td><button class="rr-btn-view" onclick="showAuthDetailModal('${item.audit_id || 'AUD-' + item.copyright_id}')">${SVG.eye} 查看详情</button></td>
                <td>${isRecovered ? `<button class="rr-btn-view" onclick="openReauthFromTabA('${item.copyright_id}')">发起重新授权</button>` : ''}</td>
              </tr>
            `;
          }).join('')}
        </tbody>
...
function openTabAExportModal() {
  const overlay = document.createElement('div');
  overlay.className = 'system-modal-overlay';
  overlay.innerHTML = `
    <div class="system-modal-container" style="max-width:420px">
      <div class="system-modal-header">
        <span class="system-modal-icon" style="color:#2563eb">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        </span>
        <span>选择导出模板</span>
      </div>
      <div class="system-modal-body" style="padding:16px 20px">
        <div style="display:flex;flex-direction:column;gap:10px">
          <label style="display:flex;align-items:center;gap:8px;padding:10px 12px;border:1px solid #e5e7eb;border-radius:8px;cursor:pointer">
            <input type="radio" name="exportTpl" value="reconciliation" checked style="accent-color:#2563eb">
            <div>
              <div style="font-weight:500;font-size:13px">二创对账明细</div>
              <div style="font-size:11px;color:#999">18个字段，用于对外对账</div>
            </div>
          </label>
          <label style="display:flex;align-items:center;gap:8px;padding:10px 12px;border:1px solid #e5e7eb;border-radius:8px;cursor:pointer">
            <input type="radio" name="exportTpl" value="rights_detail" style="accent-color:#2563eb">
            <div>
              <div style="font-weight:500;font-size:13px">引入类权益导出</div>
              <div style="font-size:11px;color:#999">37+字段，用于内部核查</div>
            </div>
          </label>
        </div>
      </div>
      <div class="system-modal-footer">
        <button class="system-modal-btn" id="exportCancel">取消</button>
        <button class="system-modal-btn system-modal-btn-primary" id="exportConfirm">确认导出</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
  overlay.querySelector('#exportCancel').onclick = () => overlay.remove();
  overlay.querySelector('#exportConfirm').onclick = () => {
    const tpl = overlay.querySelector('input[name="exportTpl"]:checked').value;
    const tplName = tpl === 'reconciliation' ? '二创对账明细（18字段）' : '引入类权益导出（37+字段）';
    overlay.remove();
    showSystemModal('导出成功', '已成功导出「' + tplName + '」（mock）', 'success');
  };
...
function renderReauthActionArea(status, data) {
    ...
    if (status === 'authorized') {
        ...
        // Not started: just the action button + remark
        html += '<div class="reauth-action-area">';
        html += '<div class="action-form-group"><label class="action-form-label">\u5907\u6ce8\uff08\u53ef\u9009\uff09</label>'
            ...
            + '<div class="action-btn-row" style="justify-content:center"><button class="btn-reauth-action" onclick="startReauthGroupConfirm()"><i data-lucide="send" style="width:14px;height:14px"></i> \u53d1\u8d77\u91cd\u6388\u6743</button></div>';
        html += '</div>';
    } else if (status === 'group_confirm') {
        // Step 2 active: left = Step1 conclusion, right = Step2 form
        html += '<div class="reauth-flow-columns">';
        html += '<div class="reauth-flow-left">' + getStep1Col(false) + '</div>';
        html += '<div class="reauth-flow-right">' + buildStep2Form() + '</div>';
        html += '</div>';
    } else if (status === 'pending_supplement') {
        // Step 3 active: left = Step1 + Step2 conclusions, right = Step3 form
        html += '<div class="reauth-flow-columns">';
        html += '<div class="reauth-flow-left">' + getStep1Col(false) + getStep2Col(false) + '</div>';
...
function submitReauthStep3() {
    ...
    if (conclusion === 'supplement_done') {
        ...
        if (currentDetailProject && currentDetailProject.detail) {
            ...
            if (work) { work.recovery_status = null; work.recovery_reason = ''; work.recovery_date = ''; }
        }
...
function renderReauthHistory(history) {
    ...
    var tableHtml = '<table class="reauth-history-table"><thead><tr>'
        + '<th>\u6b21\u6570</th><th>\u53d1\u8d77\u65f6\u95f4</th><th>\u5916\u5faa\u73af\u64cd\u4f5c\u4eba</th><th>\u8865\u51fd\u7ed3\u8bba</th><th>\u8865\u51fd\u7ed3\u8bba\u64cd\u4f5c\u4eba</th><th>\u8865\u51fd\u786e\u8ba4\u7ed3\u679c</th><th>\u8865\u51fd\u786e\u8ba4\u64cd\u4f5c\u4eba</th><th>\u5907\u6ce8</th>'
        + '</tr></thead><tbody>';
